...
bool Application::Initialize(AppCreateParams& createParams)
{
	...
	LOG_INFO(LogChannel::LC_Core, "Initializing Renderer...");
...
void Application::ProcessCMDArgs(const char* args)
{
	...
	while (dash != nullptr)
	{
		...
		uint64 strLen = (uint64)dash2 - (uint64)dash - 2;
		if (dash2 == nullptr)
			strLen = strlen(dash) - 1;
		CMDArg arg;

		// -2 for - at the begining and space at the end so we dont capture them
		Torc::Platform::MemCopy(arg.Data(), (void*)(dash+1), strLen);

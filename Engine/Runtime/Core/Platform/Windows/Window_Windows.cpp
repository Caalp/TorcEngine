...
void Torc::WindowWin64::Notify()
{
	...
	{
		TE_Error(LogChannel::LC_Platform, "PostMessage failed! %s", __FILE__)
	}
}
...
void Torc::WindowWin64::SendMessageToWindow(WindowMessage message, void* data)
{
	...
	case WindowMessage::WINDOW_MESSAGE_CLOSE_WINDOW:
		SendMessageA((HWND)m_windowHandle, WM_CLOSE, 0, 0);
		break;
	default:
		break;
	}
...
int32_t DebugWindow::CreateWnd(const WndCreateParams& params)
{
	...
	{
		DWORD error = GetLastError();
		TE_Error(LogChannel::LC_Platform, "Failed to Create DebugWindow, Invalid Handle!");
		return 0;
	}
	gEnv->logger->RegisterListener(debugWindowLogger = new DebugWindowLogger((HWND)m_windowHandle));
	ShowWindow((HWND)m_windowHandle, SW_SHOW);
	SetWindowStatus(WindowStatus::Ready);
	return 0;
}

...
bool PlatformGamepad::HasStateChanged(int idx)
{
    ...
    if (ret == ERROR_DEVICE_NOT_CONNECTED)
    {
        ...
        TE_Error(LogChannel::LC_Core, "XInput Device is not connected at %d", idx);
        return true;
    }
...
void PlatformGamepad::SetFeedback(uint16_t leftMotorSpeed, uint16_t rightMotorSpeed, int idx)
{
    ...
    if (ret != ERROR_SUCCESS)
    {
        TE_Error(LogChannel::LC_Core, "Error occured while setting gamepad feedback %d", idx);      
    }
}
...
void PlatformGamepad::GetBatteryInfo(Battery* battery, int idx)
{
    ...
    if (ret != ERROR_SUCCESS)
    {
        TE_Warning(LogChannel::LC_Core, "XInput failed to retrieve battery information for device at %d", idx);
        return;
    }
...
void PlatformGamepad::HandleDeviceChange()
{
    ...
    auto newEnd = std::remove_if(devices.begin(), devices.end(), [](XInputDevice dev){
            ...
            if (dev.isConnected == true && ret != ERROR_SUCCESS)
            {
                ...
                TE_Info(LogChannel::LC_Platform, "Gamepad is disconnected at index %d", dev.deviceIndex);
                return true;
            }
        return false;
        });
    ...
    for (int i = 0; i < XUSER_MAX_COUNT; ++i)
    {
        ...
        if (ret == ERROR_SUCCESS && devices.size() <= i)
        {
            ...
            device.isConnected = true;
            devices[i] = device;
            TE_Info(LogChannel::LC_Platform, "Gamepad is connected at index %d", i);
        }
    }
}

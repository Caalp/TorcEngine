...
TEST_CASE("Test Json", "[Json]")
{
	...
	IO::Windows::FileIO io;

	IO::FileIOBase::SetInstance(&io);

	Json::EResult res = reader.OpenStream("./engine.json");
	REQUIRE(res == Json::EResult::Success);

	Json::JsonDocument& doc = reader.ReadAndParseStream();
	auto jVal00 = doc.GetValue("PackagesDirectory");
	auto jVal01 = doc.GetValue("Double");
	auto jVal02 = doc.GetValue("bool");
	auto jVal03 = doc.GetValue("Float");
	auto jVal04 = doc.GetValue("Object");

	Std::string str0 = jVal00.String();
	double extracted_jVal01 = jVal01.Double();
	auto v = doc.GetValue("Modules");
